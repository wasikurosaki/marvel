import React from "react";
import { motion } from "framer-motion";
import adelaide from "../assets/adelaide.png";

const Partners = () => {
  const partners = [
    {
      id: 1,
      alt: "Microsoft Silver Partner",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 2,
      alt: "Salesforce Consulting Partner",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 3,
      alt: "Webmerge",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 4,
      alt: "Kentico Bronze Partner",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 5,
      alt: "Amazon Web Services Partner",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 6,
      alt: "IBM SI Partner",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 7,
      alt: "Dell Boomi",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 8,
      alt: "Cloudera Connect",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
    {
      id: 9,
      alt: "Salesforce ISV Partner",
      src: adelaide,
      className: "w-[280px] md:h-[150px] h-[100px]",
    },
  ];

  return (
    <div id="partners" className="w-full py-16 bg-[#FAF9F6]">
      <div className="max-w-7xl mx-auto px-4">
        {/* Title with orange underline */}
        <div className="text-center mb-20">
          <h1 className="md:text-[4rem] text-3xl font-marvel font-extrabold text-[#30315f]">
            Education Partners
          </h1>
        </div>

        {/* Partners Grid */}
        <motion.div
          className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-10 justify-items-center items-center"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
          {partners.map((partner) => (
            <motion.div
              key={partner.id}
              className="flex items-center justify-center"
              whileHover={{ scale: 1.05 }}
              transition={{ duration: 0.2 }}
            >
              <img
                src={partner.src}
                alt={partner.alt}
                className={`${partner.className} object-contain filter hover:brightness-110 transition-all duration-300`}
              />
            </motion.div>
          ))}
        </motion.div>
      </div>
    </div>
  );
};

export default Partners;
